...
1. **判断输入来源**
   ...
   - 只给已有 T5T → 按要求改，保留原意。
   - 无材料 → 建议补充数据来源，不直接生成。
2. **生成准则**（用户明确要求优先级最高，下列为未指定时的默认兜底，不得用默认削弱用户明确要求；唯一硬限是脚本的 ≤5 条 / ≤80 字 / 纯字符串数组格式）：最多 5 条 / 按优先级 / 真实客观 / 去琐碎；每条 ≤80 字。**润色到位**：写成完整、具体的一句话（说清进展/目标/卡点，像正式周报条目，不甩干巴短词/半截短语）；真实客观——有成绩如实说、有问题也直接提不回避；润色只作用于表达层，不堆形容词/不夸大/不编造成果/不注水，信息照实、表达讲究。本周内容按上下文判断、自然承接：提供了本周周报/进展/重点时，识别能贯穿两周的事项（本周已推进、下周继续推进/完善/上线的）写出「本周成果/节点 → 下周动作」的承接，纯下周新事项正常写；看是否真有跨周承接关系决定带不带，不是每条都套，没有本周内容则不编。**一稿成型、不要墨迹**：直接写 1～5 条交付，不反复推翻重写/列多版草稿/来回自审；别手动数字数（长度脚本兜底），条数按材料真实支撑写（够 3 条就 3 条，不硬凑 5 条），下周动作从本周工作自然顺延不算编造、不要纠结。
