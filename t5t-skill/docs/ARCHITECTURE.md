...
1. **判断输入来源**
   ...
   - 只给已有 T5T → 按要求改，保留原意。
   - 无材料 → 建议补充数据来源，不直接生成。
2. **生成准则**：最多 5 条 / 按优先级 / 真实客观 / 去琐碎；每条 ≤80 字。**润色到位**：写成完整、通顺、具体的一句话（有主语/动作/对象/结果，像正式周报条目，不甩干巴短词/半截短语）；润色只作用于表达层——说完整说通顺、用词得体，但不堆形容词/不夸大/不编造成果/不注水，信息照实、表达讲究。默认以下周重点为主；仅当用户要求、或某条本身是重点/核心进展时，才在少数关键条上适当点出本周（「本周成果/节点 → 下周继续/完善」），不是每条都带，没有本周内容则不硬套。
