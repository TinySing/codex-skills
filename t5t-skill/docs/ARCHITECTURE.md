...
1. **判断输入来源**
   ...
   - 只给已有 T5T → 按要求改，保留原意。
   - 无材料 → 建议补充数据来源，不直接生成。
2. **生成准则**（用户明确要求优先级最高，下列为未指定时的默认兜底，不得用默认削弱用户明确要求；唯一硬限是脚本的 ≤5 条 / ≤80 字 / 纯字符串数组格式）：最多 5 条 / 按优先级 / 真实客观 / 去琐碎；每条 ≤80 字。**润色到位**：写成完整、具体的一句话（说清进展/目标/卡点，书面得体，不甩干巴短词/半截短语）；真实客观——有成绩如实说、有问题也直接提不回避；润色只作用于表达层，不堆形容词/不夸大/不编造成果/不注水，信息照实、表达讲究。T5T 是「下周」要做的事、不是本周完成清单/周报：每条落点都在下周（继续推进/上线/优化/收尾/启动）。给本周周报时把「本周做了什么」转成「下周承接什么」，不能把周报条目原样润色成「完成 A、完成 B」的过去式口吻；要承接就写「本周完成 X → 下周继续/完善 Y」（本周作背景、下周是落点），纯下周新事项直接写下周动作。本周节点看有无承接关系决定带不带，下周动作每条都得有。用户说「润色/优化」现有内容时同样适用：源内容是本周完成式/周报口吻时，把它承接到下周才算合法 T5T，不算违反「润色只动表达」、不算改信息，拿不准就问一句要不要转。**一稿成型、不要墨迹**：直接写 1～5 条交付，不反复推翻重写/列多版草稿/来回自审；别手动数字数（长度脚本兜底），条数按材料真实支撑写（够 3 条就 3 条，不硬凑 5 条），下周动作从本周工作自然顺延不算编造、不要纠结。
