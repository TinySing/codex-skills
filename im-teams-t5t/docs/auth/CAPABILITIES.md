...
本文档用于快速判断 im-teams-t5t 认证子模块（`scripts/auth/`）能做什么、如何被 t5t 业务脚本调用，以及有哪些明确限制。完整实现流程和安全设计见 [ARCHITECTURE.md](./ARCHITECTURE.md)。
...
| 业务脚本集成 | 以退出码和 JSON 状态向调用方返回结果 | 认证策略只由认证子模块定义 |
