#!/usr/bin/env python3
"""
env_check.py - Cross-platform Python environment detection module for 360Teams Skills.

Core capabilities:
  - Detect available Python 3.9+ interpreter path across platforms
  - Bypass VM Shim (shims under vm/tools/ truncate Chinese arguments)
  - Cache detection results (7-day validity, stored in skill directory)
  - Check keyring availability and auto-install if missing
  - Output installation guidance when Python is not installed
...
Usage as a CLI:
    python env_check.py
    # Outputs JSON with status, python_path, version, platform, cached, keyring_available
"""
...
from config import CACHE_DIR, ensure_keyring
